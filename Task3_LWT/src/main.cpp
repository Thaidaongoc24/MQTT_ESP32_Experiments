...
namespace
{
    ...
    const char *lwtmqtt = "esp/status";
    const char *lwt_message = "Lost Connection";
...
void mqttReconnect()
{
    while (!mqttClient.connected())
    {
        ...
        if (mqttClient.connect(client_id.c_str(), MQTT::username, MQTT::password, lwtmqtt, 1 /*QoS1*/, true, lwt_message))
        {
